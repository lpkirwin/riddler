...
```{r}
sphere_plot <- function(SL, SR, SB, ST, M1, M2, phi = 55, theta = 80) {
...
sim_anneal <- function(s_pts, m, rounds) {
  ...
  for (t in 1:rounds) {
    old_vol <- volume(m)
    new_m <- m + rnorm(6)*temp/start_temp*2
    ...
    temp <- temp * 0.9999 # exponential
    if (t %% 2000 == 0) cat(paste0(t,"."))
    # if (TRUE) cat(paste0(t,"."))
  }
  return(history)
}

res <- sim_anneal(S_pts, M1, 100000)
```
...
m1 <- res2[res2$t == max_t,1:3]
m2 <- polygon_pts(m1$x, m1$y, m1$z)

```

```{r}
sphere_plot(SL, SR, SB, ST, m1, m2, -20, 20)
